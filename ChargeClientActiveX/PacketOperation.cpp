...
CPacketOperation::CPacketOperation(void)
{
}
...
bool CPacketOperation::InitInstance(void)
{
	//引导ET199的库文件，进行环境初始化
	CK_RV rv;
...
void CPacketOperation::ExitInstance(void)
{
	C_Finalize(NULL);
}
bool CPacketOperation::SelfCheck(CString &info)
{
	USES_CONVERSION;
	CString str("i an a test!");
	int strlen=str.GetLength();
	unsigned char *pstr=(unsigned char *)str.GetBuffer(strlen);
	unsigned char *pSignedBuffer=NULL;//需要清理缓冲区
	int Signedbuffer_len=0;
	//加密
	m_bServerPublic=FALSE;//强制使用本地公钥
	if(SignedEncrypt(pstr,strlen,(unsigned char**)&pSignedBuffer,Signedbuffer_len)==false)
	{
		info="测试加密失败";
		LOG(ERROR)<<T2A(info);
...
bool CPacketOperation::GetUKID(unsigned char ukid[],DWORD len,CString &info)
{
...
bool CPacketOperation::SignedEncrypt(unsigned char *in_buffer, int in_buffer_Len, unsigned char** out_buffer, int &out_buffer_Len)
{
...
//      int maxnum               信息对最大个数
//返回：int                      信息对个数
void CPacketOperation::GetErroInfo(unsigned char revbuffer[],int reclen,CString &erro)
{
...
bool CPacketOperation::DecryptVerify(unsigned char *in_buffer, int in_buffer_Len, char &RetState,unsigned char** out_buffer, int &out_buffer_Len,CString &erro)
{
	USES_CONVERSION;
	/*char * key_buffer = NULL;
	key_buffer = (char *)malloc(key_length);*/
...
CString CPacketOperation::GetTerminalID(CString &info)
{	
...
CString CPacketOperation::GetUserID(CString &info)
{	
	unsigned char ukid[UKID_LEN+1];
	memset(ukid,NULL,UKID_LEN+1);
	DWORD len=UKID_LEN;
